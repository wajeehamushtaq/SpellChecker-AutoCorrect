/*
Program: spell_checker & auto_correct
...
public:
	...
	int incorrectArrangement(string &input)
	{
		...
		if (words)
		{
			while (getline(words, line))
			{
				...
				// check sizeof word and line in word.txt
				if (len == flen)
				{
					for (int i = 1; i < Xinput.length(); i++)
						// push element back like deque
						Ninput.push_back(input[i]);
					for (int i = 1; i < flen; i++)
						Nline.push_back(Xline[i]);
					Xinput.resize(1);
					Xline.resize(1);
					// use built in function of sort
	...
	void writefile()
	{
		...
		cout << "Enter text to enter in given file\n";
